...
public class ItemImgService {
    ...
    public void saveItemImg(ItemImg itemImg, MultipartFile itemImgFile) throws Exception {
        ...
        if(!StringUtils.isEmpty(oriImgName)) {
            ...
            //사용자가 상품의 이미지를 등록했다면 저장할 경로와 파일의 이름, 파일의 배열을 파일 업로드 파라미터로 uploadFile 메소드를 호출합니다.
            ...
            //또한 application.properties에서 설정한 uploadPath 프로퍼티 경로인 "C:/shop/" 아래에 item 폴더에 이미지를 저장하므로 상품 이미지를 불러오는 경로로 "/images/item"를 붙여줍니다.
        }
    ...
    public void updateItemImg(Long itemImgId, MultipartFile itemImgFile) throws Exception {
        if(!itemImgFile.isEmpty()) { //상품 이미지를 수정한 경우 상품 이미지를 업데이트합니다.
            ItemImg savedItemImg = itemImgRepository.findById(itemImgId) //싱픔 이미지 아이디를 이욯하여 기존에 저장했던 상품 이미지 엔티티를 조회합니다.
            ...
            // => 변경된 상품 이미지 정보를 세팅해줍니다. 여기서 중요한 점은 상품 등록 때처럼 itemImgRepository.save()로직을 호출하지 않는다는 것입니다.
